...
<body>
	<form:form modelAttribute="contentModel" method="post" action="setModel">     
		...
		<br />
        绑定Array的checkbox testArray标签：<br />
        ...
        绑定Map的radiobuttons path=selectId items=testmap 标签：<br />
		<form:radiobuttons path="selectId" items="${contentModel.testMap}" />
		<br />
        绑定Map的select path=selectId item=testmap 标签：<br />
		<form:select path="selectId" items="${contentModel.testMap}" />
		<br />
        不绑定items数据直接在form:option添加的select selectId标签：<br />
